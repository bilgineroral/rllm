...
def main():
    ...
    model = LengthPredictionHead(
        ...
        n_heads=config["n_heads"], ffn_expand=config["ffn_expand"]
    )
    ...
    criterion = nn.CrossEntropyLoss(weight=torch.tensor(normalized_weights, dtype=torch.float32))
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        ...
        threshold=config["scheduler_threshold"], threshold_mode='abs', min_lr=config["scheduler_min_lr"]
    )
