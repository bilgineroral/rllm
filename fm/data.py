...
class Alphabet(object):
    ...
    def get_batch_converter(self):
        if self.use_msa:
            raise Exception("MSA theme is not supported in RLLM")
    ...
    def from_architecture(cls, name: str, theme: str = "protein") -> "Alphabet":
        if theme == "protein":
            raise Exception("Protein theme is not supported in RLLM")
            ...
            raise Exception("RNA-3mer theme is not supported in RLLM")
        ...
        if name in ("ESM-1", "protein_bert_base"):
            raise Exception("ESM-1 theme is not supported in RLLM")
            ...
            raise Exception("MSA Transformer theme is not supported in RLLM")
            standard_toks = seq_toks["toks"]
            prepend_toks = ("<cls>", "<pad>", "<eos>", "<unk>")
            append_toks = ("<mask>",)
            prepend_bos = True
            ...
            raise Exception("Invariant GVP theme is not supported in RLLM")
            standard_toks = seq_toks["toks"]
